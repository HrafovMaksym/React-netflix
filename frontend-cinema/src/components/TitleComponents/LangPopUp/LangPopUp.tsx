...
const LangPopUp: React.FC = () => {
  const langArr: string[] = ["English", "Chinese", "German", "Dutch"];
  ...
  React.useEffect(() => {
    ...
    const outsideHandleClick = (event: MouseEvent) => {
  ...
  return (
    <div className={styles.root}>
      <button
        ref={rootRef}
        className={active ? styles.activeLangBtn : styles.setActiveLangBtn}
      ...
      {active && (
        <div className={styles.langCard}>
          <ul>
            {langArr.map((el: string, id: number) => (
              <li key={id} onClick={() => onClickPopup(id)}>
                {el}
              </li>
            ))}
          </ul>
